    ...
    for k, v in ckpt.items():
        ...
        if k.startswith('pos_embed'):
            ...
            if 'attn.q.' in new_k:
                ...
                continue
            elif ('attn.proj.' in new_k) and (stage_i > 2):
                new_k = new_k.replace('proj.', 'attn.out_proj.')
            elif 'attn.sr.' in new_k:
                new_k = new_k.replace('sr.', 'sr.')
            elif ('mlp.' in new_k) and (stage_i > 2):
                string = f'{new_k}-'
                new_k = new_k.replace('mlp.', 'ffn.layers.')
                if 'fc1.weight' in new_k or 'fc2.weight' in new_k:
                    new_v = v.reshape((*v.shape, 1, 1))
...
def swin_converter_stage12(ckpt):
    ...
    for k, v in ckpt.items():
        if k.startswith('head'):
            continue
        elif k.startswith('layers.0') or k.startswith('layers.1'):
            new_v = v
            if 'downsample' in k:
                new_k = k
                if 'reduction.' in k:
        ...
        elif k.startswith('layers.2') or k.startswith('layers.3'):
            new_v = v
            if 'attn.' in k:
...
def swin_converter_stage123(ckpt):
